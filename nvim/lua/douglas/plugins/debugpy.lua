...
function M.configure(subcommand, ...)
	local entry = M.subcommand[subcommand] or vim.g.debugpy_subcommand[subcommand]
	...
	if #args < min_args then
		error(
			string.format("Debugpy: %s: not enough arguments, needs at least %d, got %d", subcommand, min_args, #args)
		)
	end

	if max_args and #args > max_args then
		error(string.format("Debugpy: %s: too many arguments, needs at most %d, got %d", subcommand, max_args, #args))
	end
